...
def _apply_channel_settings(
    creator: TwitchCreator, user: dict[str, Any],
) -> None:
    values: dict[str, str] = {}
    color: str | None = _string(user.get('primaryColorHex'))
    if color and re.fullmatch(r'[0-9a-fA-F]{6}', color):
        values['primary_color_hex'] = color.upper()
    settings: object = user.get('broadcastSettings')
    if isinstance(settings, dict) and (
        settings.get('id') is None
        or str(settings['id']) == creator.user_id
    ):
        title: str | None = _string(settings.get('title'))
        if title is not None:
            values['title'] = title
        language: str | None = _string(settings.get('language'))
        if language and language.strip():
            values['language'] = language.strip().lower()
        game: object = settings.get('game')
        if isinstance(game, dict):
            category_id: int | None = _count(game.get('id'))
...
def _apply_user(creator: TwitchCreator, user: dict[str, Any]) -> None:
    _apply_channel_settings(creator, user)
    fields: dict[str, str] = {
