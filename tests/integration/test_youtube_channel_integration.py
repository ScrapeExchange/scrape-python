...
class TestIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_youtube_channel(self) -> None:
        ...
        self.assertIsNotNone(channel.description)
        self.assertGreater(len(channel.description), 0)
        self.assertGreater(len(channel.channel_thumbnails), 0)
        self.assertGreater(len(channel.banners), 0)
        self.assertGreater(len(channel.video_ids), 0)
        self.assertIsNotNone(channel.channel_thumbnail)
        ...
        # Normalize None -> 0 for count fields so the round-trip comparison
        # matches (to_dict uses ``or 0`` for these fields)
        channel.view_count = channel.view_count or 0
        channel.subscriber_count = channel.subscriber_count or 0
        channel.video_count = channel.video_count or 0
    ...
    async def test_youtube_channel_about_page(self) -> None:
        ...
        self.assertGreater(len(channel.available_country_codes), 0)
        self.assertGreaterEqual(channel.view_count, 755841320)
        self.assertIsNotNone(channel.country)
        self.assertGreater(len(channel.external_urls), 0)
    ...
    async def test_channel_resolve_id(self) -> None:
        '''Resolve a channel ID back to its handle via InnerTube.'''
        handle: str | None = await YouTubeChannel.resolve_channel_id(
            YOUTUBE_HISTORYMATTERS_CHANNEL_ID
        )
        self.assertIsNotNone(handle)
        self.assertEqual(handle.lower(), YOUTUBE_HISTORYMATTERS_CHANNEL.lower())
